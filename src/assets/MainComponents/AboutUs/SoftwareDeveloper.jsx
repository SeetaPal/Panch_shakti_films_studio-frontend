export const SoftwareDeveloper = () => {
    return (
      
            <div className="container my-5">
                <div className="founder-container">
                    <div className="founder-title">
                    Software Developers
                    </div>
                    <div className="row g-4 justify-content-center mt-3">

                        <div className="col-12 col-md-5 col-lg-4 px-4">
                            <div className="founder-card">
                                <img src="Assets/Images/dipeeka-01.png" alt="Dipika" />
                                <div className="founder-info">
                                    <h5 className="mb-1 fw-bold text-dark">Dipika</h5>
                                    <h6 className="text-dark">Data Scientist</h6>
                                </div>
                            </div>
                        </div>

                        <div className="col-12 col-md-5 col-lg-4 px-4">
                            <div className="founder-card">
                                <img src="Assets/Images/tejaswin-01.png" alt="Tejaswin" />
                                <div className="founder-info">
                                    <h5 className="mb-1 fw-bold text-dark">Tejaswin</h5>
                                    <h6 className="text-dark">Data Scientist</h6>
                                </div>
                            </div>
                        </div>

                         <div className="col-12 col-md-5 col-lg-4 px-4">
                            <div className="founder-card">
                                <img src="Assets/Images/Pravalika-01.png" alt="Namarata Sancheti" />
                                <div className="founder-info">
                                    <h5 className="mb-1 fw-bold text-dark">Pravalika</h5>
                                    <h6 className="text-dark"> Data Scientist</h6>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
    
    )
}